...
export function Preview({ values }: { values: CustomizeValues }) {
  const paletteKey = (values.palette ?? "silver") as string;
  const colors =
    AURORA_MESH_PALETTES[paletteKey] ?? AURORA_MESH_PALETTES.silver;
  const speed = (values.speed ?? 0.3) as number;
  const grain = (values.grain ?? 0.06) as number;
  const wireframe = (values.wireframe ?? false) as boolean;
  const wireframeOpacity = (values.wireframeOpacity ?? 0.45) as number;
  const mouseFollow = (values.mouseFollow ?? true) as boolean;
  const mouseInfluence = (values.mouseInfluence ?? 0.6) as number;
  const ripple = (values.ripple ?? true) as boolean;
  const rippleStrength = (values.rippleStrength ?? 0.06) as number;
